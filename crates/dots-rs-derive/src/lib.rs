...
//!    monomorphizations of the generic `dots_rs_core::layout::opt_*` helpers.
...
//!
//! [`PropertyVtable`]: dots_rs_core::PropertyVtable
...
use syn::{
    ...
    LitStr, Path, PathArguments, Token, Type, Variant, parse_macro_input,
    parse::{Parse, ParseStream},
    spanned::Spanned,
};

/// Resolved runtime-root paths for the code a derive emits.
///
/// `#[derive(DotsStruct)]`-generated code refers to runtime items
/// (`PropertyDescriptor`, `filter::Attr`, `DotsAssign`, …). By default
/// those route through the umbrella crate's hidden `dots_rs::__rt`
/// module, so a downstream user only needs `dots-rs` as a dependency.
///
/// Two container-attribute overrides exist for crates that host the
/// runtime themselves and therefore can't depend on the umbrella:
///
/// * `#[dots(rt_internal)]` — the *defining* crate exposes its own
///   `__rt` module. Direct output uses `crate::__rt`; the companion
///   `macro_rules!` uses `$crate::__rt` so it still resolves when the
///   type is constructed from another crate (even one that only depends
///   on the umbrella, which re-exports the companion).
/// * `#[dots(rt = path::to::root)]` — use the given path verbatim for
///   both direct output and companion. Handy for integration tests in
///   the runtime crates, which depend on the leaf crate directly.
struct Roots {
    /// Prefix for items emitted directly (impls, statics, consts).
    direct: TokenStream2,
    /// Prefix for items emitted inside the per-type companion
    /// `macro_rules!` (may use `$crate`).
    companion: TokenStream2,
}

fn resolve_roots(rt_internal: bool, rt_path: &Option<Path>) -> Roots {
    if rt_internal {
        Roots {
            direct: quote! { crate::__rt },
            companion: quote! { $crate::__rt },
        }
    } else if let Some(p) = rt_path {
        Roots {
            direct: quote! { #p },
            companion: quote! { #p },
        }
        ...
        Roots {
            direct: quote! { ::dots_rs::__rt },
            companion: quote! { ::dots_rs::__rt },
        }
    }
}
...
/// ```
/// Parsed `dots!` input: an optional `rt = <path>;` runtime-root
/// override followed by the struct-literal expression.
///
/// The override is only needed by the runtime crates' own tests (which
/// can't depend on the umbrella). External callers write the bare
/// `dots!(Foo { .. })` form and the root defaults to `::dots_rs::__rt`.
struct DotsMacroInput {
    rt: TokenStream2,
    expr: syn::ExprStruct,
}

impl Parse for DotsMacroInput {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        // A leading `rt = <path> ;` is distinguished from a struct path
        // by the `=` following the first identifier (a struct literal
        // has `{` or `::` there instead).
        let rt = if input.peek(Ident) && input.peek2(Token![=]) {
            let key: Ident = input.parse()?;
            if key != "rt" {
                return Err(syn::Error::new(key.span(), "dots!: expected `rt = <path>;` override"));
            }
            input.parse::<Token![=]>()?;
        ...
        Ok(DotsMacroInput { rt, expr })
    }
}

#[proc_macro]
pub fn dots(input: TokenStream) -> TokenStream {
    let DotsMacroInput { rt, expr } = parse_macro_input!(input as DotsMacroInput);
    expand_dots_struct(&expr, &rt).into()
}

fn expand_dots_struct(expr: &syn::ExprStruct, dots_rt: &TokenStream2) -> TokenStream2 {
    ...
    if let Some(rest) = &expr.rest {
        let field_assignments = expr.fields.iter().map(|fv| {
            ...
            let value_tokens = expand_dots_field_value(&fv.expr, dots_rt);
    ...
    let field_pairs = expr.fields.iter().map(|fv| {
        let member = &fv.member;
        let value = dots_value_for_companion(&fv.expr, dots_rt);
        quote! { #member : #value }
    });
...
fn dots_value_for_companion(expr: &syn::Expr, dots_rt: &TokenStream2) -> TokenStream2 {
    match expr {
        syn::Expr::Struct(inner) => expand_dots_struct(inner, dots_rt),
        other => quote! { #other },
    }
}

fn expand_dots_field_value(expr: &syn::Expr, dots_rt: &TokenStream2) -> TokenStream2 {
    ...
    quote! {
        {
            ...
            use #dots_rt::DotsAssignGeneric as _;
            #dots_rt::DotsAssign(#inner).into_dots_field()
        }
    }
}
...
struct ContainerAttrs {
    ...
    local: bool,
    substruct_only: bool,
    /// `#[dots(rt_internal)]`: route generated paths through the
    /// defining crate's own `__rt` module (`crate::__rt` / `$crate::__rt`).
    rt_internal: bool,
    /// `#[dots(rt = path)]`: route generated paths through `path`.
    rt_path: Option<Path>,
}
...
fn expand(input: DeriveInput) -> syn::Result<TokenStream2> {
    ...
    let roots = resolve_roots(container.rt_internal, &container.rt_path);
    let dots_rt = &roots.direct;
    ...
    let property_decls = fields.iter().map(|f| property_decl(struct_ident, f, dots_rt));

    let property_descriptors = fields.iter().map(|f| {
        let name = unraw(f.ident);
        let tag = f.tag;
        let is_key = f.is_key;
        let kind = field_kind_for(f.inner_ty, dots_rt);
        let vtable_ident = vtable_ident(f.ident);
        let field_ident = f.ident;
        quote! {
            #dots_rt::PropertyDescriptor {
                name: #name,
                tag: #tag,
                is_key: #is_key,
    ...
    let flags_expr = build_flags_expr(&container, dots_rt);

    // Per-type companion macro that `dots!` delegates to (Escape B).
    let companion = companion_macro(struct_ident, &fields, &roots.companion);
    ...
    let filter_consts = fields.iter().map(|f| {
        ...
        let prop_const = quote! {
            ...
            pub const #prop_ident: #dots_rt::PropertySet =
                #dots_rt::PropertySet::EMPTY.with_tag(#tag);
        };
        if is_dsl_leaf_type(f.inner_ty) {
            ...
            quote! {
                ...
                pub const #upper: #dots_rt::filter::Attr<Self, #inner_ty> =
                    #dots_rt::filter::Attr::new(#tag);
            }
        } else {
            prop_const
        }
    ...
    let publishable_impl = if container.substruct_only {
        ...
        quote! {
            impl #dots_rt::Publishable for #struct_ident {
                fn static_descriptor(&self) -> ::core::option::Option<&'static #dots_rt::StructDescriptor> {
                    ::core::option::Option::Some(
                        <Self as #dots_rt::StructValue>::type_descriptor(),
                    )
                }
            }
        }
    ...
    let global_registration_impl = quote! {
        impl #dots_rt::GlobalRegistration for #struct_ident {
            fn register_as_published() {
                #[#dots_rt::linkme::distributed_slice(#dots_rt::PUBLISHED_TYPES)]
                #[linkme(crate = #dots_rt::linkme)]
                static REG: &'static #dots_rt::StructDescriptor = #struct_ident::DESCRIPTOR;
                let _ = &REG;
            }
            fn register_as_subscribed() {
                #[#dots_rt::linkme::distributed_slice(#dots_rt::SUBSCRIBED_TYPES)]
                #[linkme(crate = #dots_rt::linkme)]
                static REG: &'static #dots_rt::StructDescriptor = #struct_ident::DESCRIPTOR;
                let _ = &REG;
            }
    ...
    let output = quote! {
        ...
        const _: () = {
            ...
            static #descriptor_const_ident: #dots_rt::StructDescriptor =
                #dots_rt::StructDescriptor {
                    name: #wire_name,
                    flags: #flags_expr,
            ...
            impl #struct_ident {
                ...
                pub const DESCRIPTOR: &'static #dots_rt::StructDescriptor =
        ...
        impl #dots_rt::StructValue for #struct_ident {
            #[inline]
            fn descriptor(&self) -> &'static #dots_rt::StructDescriptor {
                Self::DESCRIPTOR
            }

            #[inline]
            fn type_descriptor() -> &'static #dots_rt::StructDescriptor {
                Self::DESCRIPTOR
            }

            #[inline]
            fn valid_set(&self) -> #dots_rt::PropertySet {
                #[allow(unused_mut)]
                let mut set = #dots_rt::PropertySet::EMPTY;
                #( #valid_set_arms )*
                set
            }
        ...
        // struct or an enum.
        impl #dots_rt::DotsTypeKind for #struct_ident {
            const KIND: #dots_rt::FieldKind =
                #dots_rt::FieldKind::Struct(Self::DESCRIPTOR);
        }
        ...
        impl #dots_rt::DotsField for #struct_ident {
            #[inline]
            fn dots_encode(
                &self,
                e: &mut #dots_rt::layout::CborEncoder<'_>,
            ) -> ::core::result::Result<(), #dots_rt::EncodeError> {
                #dots_rt::layout::encode_struct_value(self, e)
            }

            #[inline]
            fn dots_decode(
                d: &mut #dots_rt::layout::CborDecoder<'_>,
            ) -> ::core::result::Result<Self, #dots_rt::DecodeError> {
                #dots_rt::layout::decode_typed_from_decoder::<Self>(d)
            }
        }
...
/// `opt_*` helpers from `dots_rs_core::layout`.
///
...
fn property_decl(_struct_ident: &Ident, f: &DotsField<'_>, dots_rt: &TokenStream2) -> TokenStream2 {
    let inner_ty = f.inner_ty;
    let vtable_ident = vtable_ident(f.ident);
    ...
    if f.bare_key {
        ...
        return quote! {
            static #vtable_ident: #dots_rt::PropertyVtable = #dots_rt::PropertyVtable {
                layout: ::core::alloc::Layout::new::<#inner_ty>(),
                init: #dots_rt::layout::key_init::<#inner_ty>,
                is_set: #dots_rt::layout::key_is_set,
                encode_value: #dots_rt::layout::key_encode::<#inner_ty>,
                decode_value: #dots_rt::layout::key_decode::<#inner_ty>,
                drop_in_place: #dots_rt::layout::key_drop::<#inner_ty>,
                clone_in_place: #dots_rt::layout::key_clone::<#inner_ty>,
                set_none: #dots_rt::layout::key_set_none::<#inner_ty>,
                take_in_place: #dots_rt::layout::key_take::<#inner_ty>,
            };
        };
    }
    ...
    if let Some(elem_ty) = vec_element_type(inner_ty) {
        ...
        return quote! {
            static #vtable_ident: #dots_rt::PropertyVtable = #dots_rt::PropertyVtable {
                ...
                init: #dots_rt::layout::opt_init::<#inner_ty>,
                is_set: #dots_rt::layout::opt_is_set::<#inner_ty>,
                encode_value: #dots_rt::layout::opt_encode_vec::<#elem_ty>,
                decode_value: #dots_rt::layout::opt_decode_vec::<#elem_ty>,
                drop_in_place: #dots_rt::layout::opt_drop::<#inner_ty>,
                clone_in_place: #dots_rt::layout::opt_clone_vec::<#elem_ty>,
                set_none: #dots_rt::layout::opt_set_none::<#inner_ty>,
                take_in_place: #dots_rt::layout::opt_take::<#inner_ty>,
            };
        };
    }
    ...
    quote! {
        static #vtable_ident: #dots_rt::PropertyVtable = #dots_rt::PropertyVtable {
            ...
            init: #dots_rt::layout::opt_init::<#inner_ty>,
            is_set: #dots_rt::layout::opt_is_set::<#inner_ty>,
            encode_value: #dots_rt::layout::opt_encode::<#inner_ty>,
            decode_value: #dots_rt::layout::opt_decode::<#inner_ty>,
            drop_in_place: #dots_rt::layout::opt_drop::<#inner_ty>,
            clone_in_place: #dots_rt::layout::opt_clone::<#inner_ty>,
            set_none: #dots_rt::layout::opt_set_none::<#inner_ty>,
            take_in_place: #dots_rt::layout::opt_take::<#inner_ty>,
        };
    }
...
/// fully-qualified `dots_rs_core::AnyObject` form.
...
fn companion_macro(
    ...
    let pick_arms = fields.iter().map(|f| {
        ...
        let found_body = if f.bare_key {
            ...
            quote! { #dots_rt::__dots_into_bare($v) }
        } else {
            ...
            quote! {{
                #[allow(unused_imports)]
                use #dots_rt::DotsAssignGeneric as _;
                #dots_rt::DotsAssign($v).into_dots_field()
            }}
        };
    ...
    quote! {
        ...
        // or crate (`use dots_rs_model::*`). The two bindings collide only
...
fn parse_container_attrs(attrs: &[syn::Attribute]) -> syn::Result<ContainerAttrs> {
    ...
    for attr in attrs {
        ...
        attr.parse_nested_meta(|meta| {
            ...
            } else if meta.path.is_ident("rt_internal") {
                out.rt_internal = true;
            } else if meta.path.is_ident("rt") {
                out.rt_path = Some(meta.value()?.parse::<Path>()?);
            } else {
...
fn field_kind_for(ty: &Type, dots_rt: &TokenStream2) -> TokenStream2 {
    if let Type::Path(tp) = ty {
        if let Some(last) = tp.path.segments.last() {
            let name = last.ident.to_string();
            ...
            if let Some(p) = primitive {
                ...
                return quote! { #dots_rt::FieldKind::#kind_ident };
            }
            if name == "Vec" {
                ...
                if let Some(inner) = vec_element_type(ty) {
                    let inner_kind = field_kind_for(inner, dots_rt);
                    return quote! { #dots_rt::FieldKind::Vec(&#inner_kind) };
                }
            }
        }
    }
    ...
    quote! { <#ty as #dots_rt::DotsTypeKind>::KIND }
}

fn build_flags_expr(c: &ContainerAttrs, dots_rt: &TokenStream2) -> TokenStream2 {
    let cached = c.cached;
...
struct EnumContainerAttrs {
    name: Option<String>,
    /// See [`ContainerAttrs::rt_internal`].
    rt_internal: bool,
    /// See [`ContainerAttrs::rt_path`].
    rt_path: Option<Path>,
}
...
fn expand_enum(input: DeriveInput) -> syn::Result<TokenStream2> {
    ...
    let output = quote! {
        #[doc(hidden)]
        const _: () = {
            static #descriptor_const_ident: #dots_rt::EnumDescriptor =
                #dots_rt::EnumDescriptor {
                    name: #wire_name,
                    elements: &[
                        #( #element_inits ),*
                    ],
            ...
            impl #enum_ident {
                ...
                pub const DESCRIPTOR: &'static #dots_rt::EnumDescriptor =
                    &#descriptor_const_ident;
            }
        };

        impl #dots_rt::DotsTypeKind for #enum_ident {
            const KIND: #dots_rt::FieldKind =
                #dots_rt::FieldKind::Enum(Self::DESCRIPTOR);
        }

        impl #dots_rt::DotsField for #enum_ident {
            #[inline]
            fn dots_encode(
                &self,
                e: &mut #dots_rt::layout::CborEncoder<'_>,
