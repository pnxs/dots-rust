...
struct CachedEntry {
    ...
    last_update_time: Option<dots_rs_core::Timepoint>,
...
impl HostTransceiver {
    ...
    pub fn new(self_name: impl Into<String>) -> Arc<Self> {
        let registry = Arc::new(dots_rs_model::registry_with_internal_types());
    ...
    /// [`dots_rs_model::registry_with_internal_types`].
        ...
        T: StructValue + Clone,
    {
        ...
        let payloads: Vec<dots_rs_model::Payload> = {
            let inner = self.inner.lock().expect("host mutex poisoned");
            inner
                .pool
                .get(type_name)
        ...
        payloads
            ...
            .filter_map(|p| match p {
                ...
                dots_rs_model::Payload::Typed(a) => a.as_typed::<T>().cloned(),
                // Wire: fall back to the CBOR roundtrip (rare — only
                // for types the host has the dynamic descriptor for
                // but no compiled-in `T`).
                dots_rs_model::Payload::Wire(d) => decode_typed_from_slice::<T>(&d.encode()).ok(),
            })
            .collect()
    }
    ...
    fn payload_as_dynamic<T: StructValue>(
        ...
        Some(dots_rs_model::Payload::Typed(AnyStruct::from_struct_value_with_mask(
            value, mask,
        )))
    }
    ...
    fn cache_and_fan_out(
        ...
        header: &DotsHeader,
        raw_frame: Bytes,
        payload: Option<dots_rs_model::Payload>,
        exclude_client_id: u32,
    ) {
        let mut inner = self.inner.lock().expect("host mutex poisoned");

        let descriptor = match self.registry.lookup(type_name) {
            Some(dots_rs_model::DescriptorEntry::Struct(d)) => Some(d.clone()),
            _ => None,
        };
        ...
        let pre_merge_payload: Option<dots_rs_model::Payload> = if is_cached && has_filtered_subs {
    ...
    fn replay_cache_to(&self, client_id: u32, type_name: &str) {
        ...
        let entries: Vec<(DotsHeader, dots_rs_model::Payload)>;
        let cached_type: bool;
        {
            ...
            cached_type = matches!(
                ...
                Some(dots_rs_model::DescriptorEntry::Struct(d)) if d.flags.is_cached()
            );
    ...
    fn handle_filtered_join(
        ...
        let descriptor = match self.registry.lookup(group_name) {
            Some(dots_rs_model::DescriptorEntry::Struct(d)) => d.clone(),
            _ => {
                tracing::warn!(
                    client_id,
                    group_name,
        ...
        let empty_predicate = dots_rs_model::DotsPredicate::default();
        ...
        // guest's writer.
        let (writer, matches): (Option<SharedWriter>, Vec<(DotsHeader, dots_rs_model::Payload, Vec<u8>)>) = {
            let inner = self.inner.lock().expect("host mutex poisoned");
            ...
            let cleanup_types: Vec<String> = inner
                ...
                .filter(|name| {
                    matches!(
                        ...
                        Some(dots_rs_model::DescriptorEntry::Struct(d))
...
fn handle_connected_message(
    ...
    if type_name == "StructDescriptorData" {
        ...
        // loop sees EOF.
        match decode_typed_from_slice::<dots_rs_model::DotsMsgError>(&raw.payload) {
    ...
    let dyn_payload: Option<dots_rs_model::Payload> = if needs_dynamic {
        match (&descriptor, DynamicStruct::decode(descriptor.clone().unwrap_or_else(|| panic!("descriptor present")), &raw.payload)) {
            (Some(_), Ok(p)) => Some(dots_rs_model::Payload::Wire(p)),
            (Some(_), Err(e)) => {
                tracing::warn!(
                    client_id,
                    type_name,
...
fn handle_echo(host: &Arc<HostTransceiver>, client_id: u32, raw: &RawTransmission) {
    let echo: dots_rs_model::DotsEcho = match decode_typed_from_slice(&raw.payload) {
        Ok(e) => e,
        Err(e) => {
    ...
    // With-base `dots!` expands inline (no companion), so its runtime
    // root must be reachable from this crate; route through `dots-rs-core`
    // directly rather than the umbrella (which `dots-rs-transport` does
    // not depend on).
    let reply = dots!(rt = ::dots_rs_core; dots_rs_model::DotsEcho {
        request: false,
        ..echo
    });
    let header = dots!(DotsHeader {
...
/// statically registers via [`dots_rs_model::register_dots_internal_types`])
...
fn handle_descriptor_request(
    ...
    let req: dots_rs_model::DotsDescriptorRequest = match decode_typed_from_slice(&raw.payload) {
        Ok(r) => r,
        Err(e) => {
...
fn handle_clear_cache(host: &Arc<HostTransceiver>, raw: &RawTransmission) {
    let req: dots_rs_model::DotsClearCache = match decode_typed_from_slice(&raw.payload) {
        Ok(r) => r,
        Err(e) => {
...
/// receive path always builds [`dots_rs_model::Payload::Wire`] before
...
fn matches_payload(
    ...
    match payload {
        dots_rs_model::Payload::Wire(d) => pred.matches(d),
        dots_rs_model::Payload::Typed(_) => false,
    }
}
...
mod tests {
    use super::*;
    use dots_rs_core::{DynamicPropertyDescriptor, StructFlags};
    ...
    fn real_io_and_framing_errors_still_error() {
        ...
        let framing = crate::TransportError::Framing(dots_rs_model::FramingError::InvalidSizePrefix(0));
