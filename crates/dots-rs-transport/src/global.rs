...
//!
//! ```ignore
//! use dots_rs_transport::{App, global as dots};
//!
...
use dots_rs_model::filter::DotsFilter;
...
pub(crate) fn init(transceiver: Arc<GuestTransceiver>) {
    ...
    if slot.is_some() {
        panic!(
            "dots_rs_transport::global: an App is already active in this process. \
             Drop the existing App (or call dots_rs_transport::global::destroy) \
...
pub fn client() -> Arc<GuestTransceiver> {
    try_client().expect(
        "dots_rs_transport::global: no App is currently constructed. \
