...
use dots_rs_model::{DotsConnectionState, DotsHeader, DotsMsgConnect, DotsMsgConnectResponse, DotsMsgHello, DotsServerCapabilities, Registry, Transmission, encode_transmission_into, encode_transmission_with_mask_into, DotsCacheInfo};
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
        ...
        T: StructValue + Clone,
    {
        ...
        match &txn.payload {
            ...
            dots_rs_model::Payload::Typed(a) => a
                .as_typed::<T>()
                .cloned()
            ...
            // CBOR roundtrip.
            dots_rs_model::Payload::Wire(d) => decode_typed_from_slice(&d.encode())
...
fn dispatch_external_owned(dispatch: &Arc<Mutex<DispatchState>>, txn: Transmission) {
    let movable = matches!(txn.payload, dots_rs_model::Payload::Typed(_))
...
/// message).
fn dispatch_keep(r: Result<bool, dots_rs_core::DecodeError>, type_name: &str) -> bool {
    match r {
        Ok(retain) => retain,
        Err(e) => {
            tracing::warn!(
...
pub(crate) trait DispatchEntry: Send {
    ...
    fn dispatch(
        ...
        shared: &mut DispatchShared,
    ) -> Result<bool, dots_rs_core::DecodeError>;
    ...
    T: StructValue + Send + Sync + Clone + 'static,
{
    fn dispatch(
        ...
        let value: T = match &txn.payload {
            dots_rs_model::Payload::Typed(a) => match a.as_typed::<T>() {
                Some(t) => t.clone(),
                ...
                None => return Err(dots_rs_core::DecodeError::message(
                    "typed dispatch: payload descriptor identity didn't match T",
                )),
            },
            dots_rs_model::Payload::Wire(d) => {
                let bytes = d.encode();
