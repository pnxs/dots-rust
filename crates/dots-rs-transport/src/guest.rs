...
impl GuestTransceiver {
    ...
    where
        T: StructValue + Send + Sync + Clone + 'static + dots_rs_core::GlobalRegistration,
    {
    ...
    /// [`DynamicStruct::try_as_publishable`](dots_rs_core::DynamicStruct::try_as_publishable))
    ...
    /// [`Registry::build_dynamic_struct`](dots_rs_model::Registry::build_dynamic_struct).
    ...
    where
        T: StructValue + Send + Sync + 'static + dots_rs_core::GlobalRegistration,
    {
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    pub async fn early_subscribe(&mut self) -> Result<(), GuestError> {
        ...
        // subscriptions).
        if conn.state() == dots_rs_model::DotsConnectionState::EarlySubscribe {
    ...
    F: FnMut(&Event<DynamicStruct>) + Send + 'static,
{
    fn dispatch(
        ...
        let dyn_value = match &txn.payload {
            dots_rs_model::Payload::Wire(d) => d.clone(),
            dots_rs_model::Payload::Typed(a) => DynamicStruct::from_struct_value(a),
        };
