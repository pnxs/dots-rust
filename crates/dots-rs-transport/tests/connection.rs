...
use dots_rs_core::{StructValue, dots, encode_to_vec};
...
use dots_rs_transport::{Connection, ConnectionBuilder, ConnectionError, TransmissionCodec};
...
fn dynamic_for(reg: &Registry, type_name: &str, payload: &dyn StructValue) -> Transmission {
    ...
    let payload = dots_rs_core::DynamicStruct::decode(descriptor, &bytes).unwrap();
    Transmission { header, payload: dots_rs_model::Payload::Wire(payload) }
}
...
async fn establish_passes_client_name_to_server() {
    ...
    let server = tokio::spawn(async move {
        ...
        let connect: DotsMsgConnect = dots_rs_core::decode_typed_from_slice(&bytes).unwrap();
...
async fn send_typed_after_handshake_reaches_server() {
    ...
    let server = tokio::spawn(async move {
        ...
        let decoded: Demo = dots_rs_core::decode_typed_from_slice(&bytes).unwrap();
