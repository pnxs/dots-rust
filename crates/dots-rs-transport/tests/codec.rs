...
use bytes::BytesMut;
use dots_rs_core::{DynamicStruct, decode_typed_from_slice, dots, encode_to_vec};
...
use dots_rs_transport::{TransmissionCodec, TransportError};
...
async fn typed_payload_decodes_via_registry_then_back_to_typed() {
    ...
    let frame = dots_rs_model::encode_transmission(&header, &typed_payload);
