...
pub fn generate_with_imports(
    ...
    match (has_struct, has_enum) {
        (true, true) => out.push_str("use dots_rs::{DotsEnum, DotsStruct};\n"),
        (true, false) => out.push_str("use dots_rs::DotsStruct;\n"),
        (false, true) => out.push_str("use dots_rs::DotsEnum;\n"),
        (false, false) => {}
    }
...
fn map_primitive(name: &str) -> &str {
    match name {
        ...
        "float64" => "f64",
        "string" => "String",
        "timepoint" | "steady_timepoint" => "dots_rs::Timepoint",
        "duration" => "dots_rs::Duration",
        "property_set" => "u64",
        "uuid" => "[u8; 16]",
...
mod tests {
    ...
    fn temporal_types_resolve_to_dots_core() {
        ...
        let file = parse_str(src).unwrap();
        let out = generate(&file);
        assert!(out.contains("Option<dots_rs::Timepoint>"));
        assert!(out.contains("Option<dots_rs::Duration>"));
    }
