...
/// it as `::dots_rs_core::linkme` without users adding it as a direct
/// dep.
pub use linkme;
...
/// `dots_rs_core::minicbor` over adding minicbor directly to your `Cargo.toml`.
pub use minicbor;
...
mod macro_tests {
    ...
    fn dots_macro_recurses_into_nested_struct_literal() {
        let w = dots!(rt = ::dots_rs_core; Wrap {
            tag: "outer",
    ...
    fn dots_macro_constructs_partial_struct() {
        let foo = dots!(rt = ::dots_rs_core; Foo { id: 42, name: "hi", ..Default::default() });
    ...
    fn dots_macro_handles_typed_integer_literals() {
        let foo = dots!(rt = ::dots_rs_core; Foo { big_id: 9000_u64, flag: true, ..Default::default() });
    ...
    fn dots_macro_supports_trailing_comma() {
        let foo = dots!(rt = ::dots_rs_core; Foo { id: 1, ..Default::default() });
        assert_eq!(foo.id, Some(1));
    }

    #[test]
    fn dots_macro_with_no_fields_yields_default() {
        let foo = dots!(rt = ::dots_rs_core; Foo { ..Default::default() });
    ...
    fn dots_macro_passes_option_through() {
        ...
        let foo = dots!(rt = ::dots_rs_core; Foo { big_id: upstream, ..Default::default() });
        assert_eq!(foo.big_id, Some(7));

        let cleared: Option<u64> = None;
        let foo = dots!(rt = ::dots_rs_core; Foo { big_id: cleared, ..Default::default() });
        assert_eq!(foo.big_id, None);
    }
    ...
    fn dots_macro_inner_into_on_option() {
        ...
        let foo = dots!(rt = ::dots_rs_core; Foo { name: upstream, ..Default::default() });
        assert_eq!(foo.name.as_deref(), Some("hi"));
    }
}
