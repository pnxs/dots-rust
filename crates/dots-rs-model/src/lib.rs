...
// Runtime root for this crate's own `#[derive(DotsStruct)]` types, which
// carry `#[dots(rt_internal)]`. The derive emits `crate::__rt::…` (and
// `$crate::__rt::…` in companion macros) instead of routing through the
// `dots-rs` umbrella, so `dots-rs-model` stays free of a dependency on
// the umbrella that re-exports it. It re-exports the core runtime plus
// this crate's own `filter` module (which provides `filter::Attr`).
#[doc(hidden)]
pub mod __rt {
