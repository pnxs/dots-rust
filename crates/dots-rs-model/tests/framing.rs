...
use dots_rs_core::encode_to_vec;
use bytes::Bytes;
use dots_rs_model::{
...
fn back_to_back_frames_decode_with_correct_offsets() {
    ...
    let p1_back: Sample =
        dots_rs_core::decode_typed_from_slice(&t1.payload.encode()).unwrap();
    let p2_back: Sample =
        dots_rs_core::decode_typed_from_slice(&t2.payload.encode()).unwrap();
...
fn raw_transmission_decode_payload_matches_typed() {
    ...
    let recovered: Sample = dots_rs_core::decode_typed_from_slice(&payload_bytes).unwrap();
...
fn raw_transmission_rewrite_header_keeps_payload_bytes() {
    ...
    let new_header = DotsHeader {
        ...
        sender: Some(99),
        server_sent_time: Some(dots_rs_core::Timepoint(123.0)),
        ..Default::default()
    };
